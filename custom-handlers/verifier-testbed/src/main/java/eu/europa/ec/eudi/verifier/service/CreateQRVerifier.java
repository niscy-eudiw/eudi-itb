...
@Component
public class CreateQRVerifier implements MessagingService {

  /** Logger. */
  private static final Logger LOG = LoggerFactory.getLogger(CreateQRVerifier.class);
  ...
  /**
   * The purpose of the getModuleDefinition call is to inform its caller on how the service is
   * supposed to be called.
   *
   * <p>Note that defining the implementation of this service is optional, and can be empty unless
   * you plan to publish the service for use by third parties (in which case it serves as
   ...
   */
  @Override
  public GetModuleDefinitionResponse getModuleDefinition(com.gitb.ms.Void parameters) {
   ...
   *
   * <p>This call expects from the service to do the following:
   *
   * <ul>
   *   <li>Record the session identifier to keep track of messages linked to the test session.
   *   <li>Process, if needed, the configuration provided by the SUT.
   *   <li>Return, if needed, configuration to be displayed to the user for the SUT actor.
   * </ul>
   *
   ...
   *
   * <p>The goal here is to be informed by the test bed on the characteristics of the message we are
   * expecting to receive. These characteristics would need to be recorded as part of this operation
   * in the service's session state so that incoming messages can be matched against them. Once the
   * expected message is received, the TestBedNotifier can then be used to ping the test bed.
   *
   * <p>Besides the expected message's characteristics, the service should also record:
   *
   * <ul>
   *   <li>The test session identifier.
   *   <li>The call identifier (the identifier of the relevant 'receive' step that resulted in this
   *       call).
   *   <li>The callback address of the test bed (this could also be fixed as a configuration
   *       property).
   * </ul>
   *
   ...
   */
  @Override
  public com.gitb.ms.Void receive(ReceiveRequest parameters) {
    LOG.info(
        "Received 'receive' command from test bed for session [{}]", parameters.getSessionId());
    return new com.gitb.ms.Void();
  }
  ...
  public SendResponse send(SendRequest parameters) {
    ...
    try {
      ...
      LOG.info("Base64 encoded QR Code: {}", base64Img);
   ...
   *
   * <p>Often there is no need to take any action here but it could be interesting to do so if you
   * need specific actions per transaction.
   *
   * @param parameters The transaction configuration.
   * @return A void result.
   */
  @Override
  public com.gitb.ms.Void beginTransaction(BeginTransactionRequest parameters) {
    LOG.info("Transaction starting for session [{}]", parameters.getSessionId());
    return new com.gitb.ms.Void();
  }

  /**
   * The endTransaction operation is the counterpart of the beginTransaction and is called when the
   ...
   */
  @Override
  public com.gitb.ms.Void endTransaction(BasicRequest parameters) {
   ...
   *
   * <p>A typical action that needs to take place here is the cleanup of any resources that were
   * specific to the session in question. This would typically involve the state recorded for the
   * session.
   *
   ...
   */
  @Override
  public com.gitb.ms.Void finalize(FinalizeRequest parameters) {
